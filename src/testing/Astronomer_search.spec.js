...
describe("Testing the searchfunction", () => {
    ...
    it("search for one specific Asteroid", () => {
      ...
      // do that again to close the detailsview of the Asteroid
      cy.get(".example-element-row > .cdk-column-name").first().click();
    });

    
    it("search for a timespan", () => {
      // clear the input search field to ensure there won't be any conflicts
      cy.get("#mat-input-2").clear();
      cy.get('#mat-input-0').type('2021-06-01');
      cy.get('#mat-input-1').type('2021-06-07');
      cy.get('.mat-toolbar > :nth-child(2) > :nth-child(4)').click();
      });
  });